 ...
 *   cmake -DACINQ_LIB_DIR=/path/to/acinq/libsecp256k1-jni.so ..
 *   make
 *   ./bench_vs_acinq
 */
...
int main(void) {
    ...
    /* Messages */
    uint8_t msg[32];
    secp256k1c_sha256(msg, (const uint8_t *)"benchmark msg", 13);
    ...
    printf("%-30s %12s %12s %10s\n", "Operation", "ACINQ (us)", "Ours (us)", "Speedup");
    printf("---------------------------------------------------------------------\n");
    ...
    for (int batch_size = 4; batch_size <= 200; batch_size = (batch_size < 64) ? batch_size * 2 : 200) {
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            secp256k1c_sha256(msgs_arr[i], seed, 4);
        ...
        if (batch_size == 64) batch_size = 100;
    }
